...
function metricasAdministrativasAtenea(PDO $pdo,array $p): array
{
    ...
    $q=$pdo->prepare("SELECT COUNT(*) pedidos,SUM(payment_status='paid' AND estado IN('pagado','preparando','enviado','entregado')) pagados,SUM(estado='pendiente_pago') pendientes,SUM(estado='entregado') entregados FROM pedidos WHERE created_at>=:d AND created_at<:h");$q->execute(['d'=>$p['inicio'],'h'=>$p['fin']]);$r['kpi']=$q->fetch();
    ...
    $q=$pdo->prepare('SELECT estado etiqueta,COUNT(*) total FROM pedidos WHERE created_at>=:d AND created_at<:h GROUP BY estado ORDER BY total DESC');$q->execute(['d'=>$p['inicio'],'h'=>$p['fin']]);$r['pedidos_estado']=$q->fetchAll();
    ...
    $r['contenido']=['capacitaciones'=>(int)$pdo->query("SELECT COUNT(*) FROM elementos_seccion e JOIN secciones s ON s.id=e.seccion_id WHERE s.clave='capacitaciones' AND e.activo=1")->fetchColumn(),'productos'=>(int)$pdo->query('SELECT COUNT(*) FROM productos WHERE activo=1 AND disponible=1 AND eliminado_at IS NULL')->fetchColumn(),'pendientes'=>(int)$pdo->query('SELECT COUNT(*) FROM elementos_seccion WHERE activo=0')->fetchColumn(),'stock_bajo'=>$pdo->query('SELECT id,nombre,stock,stock_reservado,stock_minimo FROM productos WHERE activo=1 AND eliminado_at IS NULL AND GREATEST(stock-stock_reservado,0)<=stock_minimo ORDER BY (stock-stock_reservado) ASC LIMIT 10')->fetchAll()];
    $r['alertas']=['pedidos_pendientes'=>(int)$pdo->query("SELECT COUNT(*) FROM pedidos WHERE estado='pendiente_pago'")->fetchColumn(),'errores_nuevos'=>(int)$pdo->query("SELECT COUNT(*) FROM errores_sistema WHERE estado='nuevo'")->fetchColumn(),'correos_fallidos'=>(int)$pdo->query("SELECT COUNT(*) FROM correo_envios WHERE estado='fallido'")->fetchColumn()];
    return $r;
}
