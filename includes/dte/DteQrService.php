...
class DteQrService
{
    public static function buildConsultaPayload(array $dtePayload, array $responsePayload, array $settings): string
    {
        ...
        return 'ATENEA-DTE-CONSULTA|modo=' . strtoupper($modo) . '|codigoGeneracion=' . $codigoGeneracion . '|fechaEmision=' . $fechaEmision . '|selloRecibido=' . $selloRecibido;
    }
    ...
    public static function renderToPdf(TCPDF $pdf, string $payload, float $x, float $y, float $size, string $label): void
    {
        ...
        try {
            $pdf->write2DBarcode($payload, 'QRCODE,H', $x, $y, $size, $size, $style, 'N');
