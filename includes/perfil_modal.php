...
function renderizarModalPerfil(string $contexto = 'website'): void
{
    ...
    ?>
    <div class="modal fade cuenta-modal" id="modalPerfil" tabindex="-1" aria-labelledby="modalPerfilTitulo" aria-hidden="true" data-auto-open="<?= !empty($estado['abrir']) ? '1' : '0' ?>">
      <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><div><h2 class="modal-title h4 mb-1" id="modalPerfilTitulo">Mi perfil</h2><p class="mb-0 text-muted small"><?= atenea_e((string)$perfil['correo']) ?> · <?= atenea_e(etiquetaRol((string)$perfil['rol'])) ?></p></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button></div>
        <div class="modal-body">
          ...
          <div class="row g-4">
            <div class="col-lg-3"><div class="cuenta-resumen text-center"><img id="vistaPreviaFoto" src="<?= atenea_e(rutaFotoPerfil($perfil)) ?>" class="cuenta-foto" alt="Foto de perfil"><h3 class="h5 mt-3 mb-1"><?= atenea_e(trim((string)$perfil['nombre'].' '.(string)$perfil['apellido'])) ?></h3><span class="badge bg-primary"><?= atenea_e(etiquetaRol((string)$perfil['rol'])) ?></span><dl class="text-start small mt-4 mb-0"><dt>Registro</dt><dd><?= date('d/m/Y',strtotime((string)$perfil['created_at'])) ?></dd><dt>Estado</dt><dd><?= atenea_e(ucfirst((string)$perfil['estado'])) ?></dd><dt>Google</dt><dd><?= !empty($perfil['google_id'])?'Vinculada':'No vinculada' ?></dd></dl></div></div>
            <div class="col-lg-9">
              ...
              <div class="tab-content">
                <div class="tab-pane fade show active" id="cuenta-personal"><form method="post" enctype="multipart/form-data" action="<?= atenea_url('src/cuenta/actualizar-perfil.php') ?>" class="row g-3 cuenta-form-perfil"><input type="hidden" name="csrf_token" value="<?= atenea_e(obtenerTokenCsrf()) ?>"><input type="hidden" name="retorno" value="<?= atenea_e($retorno) ?>">
                  ...
                  <div class="col-12"><label class="form-label" for="cuenta_foto">Fotografía</label><input class="form-control" type="file" id="cuenta_foto" name="foto" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp"><div class="form-text">JPG, PNG o WEBP; máximo 3 MB.</div><?= $error('foto') ?></div>
