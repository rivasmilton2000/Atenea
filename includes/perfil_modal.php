...
function renderizarModalPerfil(string $contexto = 'website'): void
{
    ...
    $preferenciasCorreo = ['academico'=>1,'comunicaciones'=>1,'comercio'=>1,'novedades'=>1];
    try {
        $consultaPreferencias = obtenerConexion()->prepare('SELECT categoria,correo_habilitado FROM notificacion_preferencias WHERE usuario_id=:id');
        $consultaPreferencias->execute(['id'=>(int)$perfil['id']]);
        foreach($consultaPreferencias->fetchAll() as $preferencia) if(array_key_exists($preferencia['categoria'],$preferenciasCorreo)) $preferenciasCorreo[$preferencia['categoria']] = (int)$preferencia['correo_habilitado'];
    } catch(Throwable) {}
      ...
      <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable"><div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="row g-4">
            ...
            <div class="col-lg-9">
              <ul class="nav nav-pills cuenta-tabs mb-4" role="tablist"><li class="nav-item"><button class="nav-link active" data-bs-toggle="pill" data-bs-target="#cuenta-personal" type="button">Información personal</button></li><li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#cuenta-direccion" type="button">Dirección</button></li><li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#cuenta-seguridad" type="button">Seguridad</button></li><li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#cuenta-notificaciones" type="button">Notificaciones</button></li><li class="nav-item"><button class="nav-link" data-bs-toggle="pill" data-bs-target="#cuenta-vinculada" type="button">Cuenta vinculada</button></li></ul>
              <div class="tab-content">
                ...
                <div class="tab-pane fade" id="cuenta-notificaciones"><h3 class="h5">Preferencias de correo</h3><p class="text-muted">Los mensajes de seguridad, acceso y cambios de cuenta siempre permanecen habilitados.</p><form method="post" action="<?=atenea_url('src/cuenta/preferencias-notificaciones.php')?>"><input type="hidden" name="csrf_token" value="<?=atenea_e(obtenerTokenCsrf())?>"><input type="hidden" name="retorno" value="<?=atenea_e($retorno)?>"><?php foreach(['academico'=>'Actividad académica','comunicaciones'=>'Mensajes y comunicaciones','comercio'=>'Compras y pedidos','novedades'=>'Noticias y novedades'] as$clave=>$etiqueta):?><div class="form-check form-switch mb-3"><input class="form-check-input" type="checkbox" role="switch" id="preferencia_<?=$clave?>" name="categorias[]" value="<?=$clave?>" <?=$preferenciasCorreo[$clave]?'checked':''?>><label class="form-check-label" for="preferencia_<?=$clave?>"><?=atenea_e($etiqueta)?></label></div><?php endforeach;?><button class="btn btn-primary">Guardar preferencias</button></form></div>
