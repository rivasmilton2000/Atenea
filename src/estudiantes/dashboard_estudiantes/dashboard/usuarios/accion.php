...
try {
    if ($accion === 'revelar_sensible') {
        ...
        $q=$pdo->prepare("UPDATE user_deletions SET status='restaurada',restored_by=:admin,restored_at=NOW() WHERE user_id=:id AND status IN('desactivada','periodo_gracia')");$q->execute(['admin'=>$actorId,'id'=>$id]);
