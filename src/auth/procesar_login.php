...
try{$q=obtenerConexion()->prepare('SELECT id,nombre,apellido,correo,password,rol,foto,estado,fecha_nacimiento,dui,codigo_telefono,telefono,departamento_id,municipio_id,distrito_id FROM usuarios WHERE correo=:correo LIMIT 1');$q->execute(['correo'=>$correo]);$u=$q->fetch();
