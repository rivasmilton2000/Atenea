...
require_once dirname(__DIR__,3).'/includes/admin_notification_service.php';
...
try{
    ...
    notificarAdministracionAtenea('pedido_'.$nuevo,'Pedido '.$pedido['numero'].' '.$nuevo,$nota?:'Estado actualizado por administración.',in_array($nuevo,['cancelado','reembolsado'],true)?'advertencia':'informacion',null,atenea_url('src/dashboard/pedidos/detalle.php?id='.$id),'pedido:'.$id.':estado:'.$nuevo,['category'=>'pedidos','pedido_id'=>$id,'created_by'=>(int)$_SESSION['usuario_id']],$pdo);
