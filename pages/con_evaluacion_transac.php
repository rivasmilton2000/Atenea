        ...
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                ...
                if (" . ($redirect ? "true" : "false") . ") {
                    window.location = 'con_evaluacion.php';
                } else {
                    window.history.back();
                }
            });
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['material']) && $_FILES['material']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($fileTmpName, $newFilePath)) {
            ...
            $query = "INSERT INTO contenidos (titulo, descripcion, material, da_id) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $query);
            mysqli_stmt_bind_param($stmt, "sssi", $titulo, $descripcion, $uniqueFileName, $da_id);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                unlink($newFilePath); // Eliminar archivo si hay un error
            }
