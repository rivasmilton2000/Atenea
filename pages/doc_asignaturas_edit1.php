...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT profesor_id, materia_id, grado_id, periodo_id FROM docentes_asignaturas WHERE da_id = ?";
    $stmt = mysqli_prepare($db, $query);
    mysqli_stmt_bind_param($stmt, "i", $da_id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $current_profesor_id, $current_materia_id, $current_grado_id, $current_periodo_id);
    mysqli_stmt_fetch($stmt);
    mysqli_stmt_close($stmt);

    // Verificar si se han hecho cambios
    if ($profesor_id == $current_profesor_id && $materia_id == $current_materia_id && $grado_id == $current_grado_id && $periodo_id == $current_periodo_id) {
        ...
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "iiiii", $profesor_id, $materia_id, $grado_id, $periodo_id, $da_id);
