...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['material']) && $_FILES['material']['error'] === UPLOAD_ERR_OK) {
        ...
        // Verificar que el archivo no sea una imagen
        ...
        if (!in_array($fileExtension, $imageExtensions)) {
            ...
            showAlert('error', 'Error', 'No se permiten archivos de imagen.');
        }
    }
    ...
    if ($oldData['titulo'] != $titulo || $oldData['descripcion'] != $descripcion || 
        $oldData['da_id'] != $da_id || ($fileUploaded && $oldData['material'] != $uniqueFileName)) {
        $changes = true;
    }
