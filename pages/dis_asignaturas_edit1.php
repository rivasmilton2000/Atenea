...
$query_current = 'SELECT doc_asi_id, periodo_id FROM estudiantes_docentes WHERE ed_id = ? AND estudiante_id = ?';
...
mysqli_stmt_bind_result($stmt_current, $current_asignatura_id, $current_periodo_id);
mysqli_stmt_fetch($stmt_current);
mysqli_stmt_close($stmt_current);

// Verificar si se realizaron cambios
if ($asignatura_id == $current_asignatura_id && $periodo_id == $current_periodo_id) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location = 'dis_asignaturas.php';
                }
            });
        });
    </script>
...
$query = 'UPDATE estudiantes_docentes SET doc_asi_id = ?, periodo_id = ? WHERE ed_id = ? AND estudiante_id = ?';
$stmt = mysqli_prepare($db, $query);
mysqli_stmt_bind_param($stmt, "iiii", $asignatura_id, $periodo_id, $ed_id, $estudiante_id);
