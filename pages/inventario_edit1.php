...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT articulo, cantidad FROM inventario WHERE i_id = ?";
    $stmt = mysqli_prepare($db, $query);
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $current_articulo, $current_cantidad);
    mysqli_stmt_fetch($stmt);
    mysqli_stmt_close($stmt);

    // Verificar si se han hecho cambios
    if ($articulo == $current_articulo && $cantidad == $current_cantidad) {
    ...
    $query = "UPDATE inventario SET articulo = ?, cantidad = ? WHERE i_id = ?";
    $stmt = mysqli_prepare($db, $query);
    mysqli_stmt_bind_param($stmt, "sii", $articulo, $cantidad, $id);
