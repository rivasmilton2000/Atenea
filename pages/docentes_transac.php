            ...
            if ($count > 0) {
                ...
                if ($count > 0) {
                    ...
                    try {
                        ...
                        $stmt = $db->prepare("INSERT INTO location (PROVINCE, CITY) VALUES (?, ?)");
                        $stmt->bind_param("ss", $prov, $cit);
                        $stmt->execute();
                        $location_id = $db->insert_id;
                        $stmt->close();
                        
                        // Insertamos el docente
                        $stmt = $db->prepare("INSERT INTO employee (FIRST_NAME, LAST_NAME, GENDER, EMAIL, PHONE_NUMBER, JOB_ID, HIRED_DATE, LOCATION_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                        $stmt->bind_param("sssssssi", $fname, $lname, $gen, $email, $phone, $jobb, $hdate, $location_id);
                        $stmt->execute();
                        $stmt->close();
                        ...
                        $db->rollback();
                        $alertTitle = 'Error';
                        $alertText = 'Error al insertar el docente en la base de datos: ' . $e->getMessage();
                        $alertIcon = 'error';
                    }
                }
            }
