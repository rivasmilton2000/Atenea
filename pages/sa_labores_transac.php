...
if ($_GET['action'] === 'add') {
    ...
    if ($mdj < $current_date) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Insertar el nuevo trabajo con 'Incompleto' en los campos hour y date, e incluir j_estado
            $insertJobQuery = "INSERT INTO jobs (employee, job, description, status, hour, date, maxhour, maxdate, j_estado) 
                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($db, $insertJobQuery);
            mysqli_stmt_bind_param($stmt, "ssssssssi", $ej, $jj, $dj, $status, $hora, $fecha, $mhj, $mdj, $j_estado);
