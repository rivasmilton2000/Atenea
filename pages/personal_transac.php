...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!in_array($email_domain, $allowed_domains)) {
        ...
        $alertText = 'La fecha de contratación de este empleado es inválida.';
        $alertIcon = 'warning';
    } else {
        ...
        if ($count > 0) {
            ...
            if ($count > 0) {
                ...
                // Verificamos si el empleado ya existe por nombre completo
                ...
                if ($count > 0) {
                    $alertTitle = 'Empleado Existente';
                    $alertText = 'El empleado ' . htmlspecialchars($fname) . ' ' . htmlspecialchars($lname) . ' ya existe en el sistema. Por favor, ingrese un empleado diferente.';
                    $alertIcon = 'warning';
                } else {
                    ...
                    $stmt = $db->prepare("INSERT INTO employee (EMPLOYEE_ID, FIRST_NAME, LAST_NAME, GENDER, EMAIL, PHONE_NUMBER, JOB_ID, HIRED_DATE, LOCATION_ID) VALUES (Null, ?, ?, ?, ?, ?, ?, ?, (SELECT MAX(LOCATION_ID) FROM location))");
                    $stmt->bind_param("sssssss", $fname, $lname, $gen, $email, $phone, $jobb, $hdate);

                    if ($stmt->execute()) {
                        $alertTitle = 'Éxito';
                        $alertText = 'Empleado agregado exitosamente.';
                        $alertIcon = 'success';
                    } else {
                        $alertTitle = 'Error';
                        $alertText = 'Error al insertar el empleado en la base de datos: ' . $stmt->error;
                        $alertIcon = 'error';
                    }
                    $stmt->close();
                }
