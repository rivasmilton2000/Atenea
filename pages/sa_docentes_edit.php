...
$query = "SELECT e.EMPLOYEE_ID, e.FIRST_NAME, e.LAST_NAME, e.GENDER, e.EMAIL, e.PHONE_NUMBER, j.JOB_TITLE, e.HIRED_DATE, t.TYPE, l.PROVINCE, l.CITY, e.LOCATION_ID, e.E_ESTADO
          FROM employee e
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $g = $row['PHONE_NUMBER'];
    $h = $row['JOB_TITLE'];
    $i = date("d-m-Y", strtotime($row['HIRED_DATE']));
    $j = $row['PROVINCE'];
    $k = $row['CITY'];
    $l = $row['TYPE'];
    $estado = $row['E_ESTADO'];
...
<center>
    <div class="card shadow mb-4 col-xs-12 col-md-8 border-bottom-primary">
            <div class="card-header py-3">
              <h4 class="m-2 font-weight-bold text-primary">Editar cuenta de docente</h4>
            </div><a  type="button" class="btn btn-primary bg-gradient-primary btn-block" href="sa_docentes.php?"> <i class="fas fa-flip-horizontal fa-fw fa-share"></i> Regresar </a>
            <div class="card-body">
      

            <form role="form" method="post" action="sa_docentes_edit1.php">
              ...
              <div class="form-group row text-left text-warning">
                ...
                <div class="col-sm-9">
                  <input class="form-control" placeholder="Nombres" minlength="5" maxlength="40" name="firstname" value="<?php echo $a; ?>" required>
                </div>
              </div>
              <div class="form-group row text-left text-warning">
                <div class="col-sm-3" style="padding-top: 5px;">
                ...
                <div class="col-sm-9">
                  <input class="form-control" placeholder="Apellidos" minlength="5" maxlength="40" name="lastname" value="<?php echo $b; ?>" required>
                </div>
              </div>
...
function updateProvince(selectElement) {
    ...
    provinceInput.value = selectedOption.getAttribute('data-province');
              ...
              <div class="form-group row text-left text-warning">
                ...
                <div class="col-sm-9">
                  <input class="form-control" placeholder="Fecha de contratación" name="hireddate" readonly value="<?php echo $i; ?>" required>
                </div>
              </div>
    ...
    <div class="col-sm-9">
        <select class="form-control" name="status" required>
            <option value="" disabled <?php if($estado === null) echo 'selected'; ?>>Seleccionar estado</option>
