...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $evaluacion_estado = intval($_POST['evaluacion_estado']);
    ...
    if ($porcentaje < 1 || $porcentaje > 100) {
        $errors[] = 'El porcentaje de evaluación de esta evaluación es inválida.';
    }
    ...
    foreach ($evaluacion as $key => $value) {
        if ($key != 'evaluacion_id' && $$key != $value) {
            $changes = true;
            break;
        }
    }
    ...
    if (empty($errors)) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $errors[] = 'Error al actualizar la evaluación: ' . mysqli_stmt_error($stmt);
            }

            mysqli_stmt_close($stmt);
        } else {
            $errors[] = 'Error al preparar la consulta: ' . mysqli_error($db);
        }
    }

    if (!empty($errors)) {
        showAlert('error', 'Error', implode('<br>', $errors));
    }
} else {
