...
try{
 ...
 $q=$pdo->prepare('SELECT COUNT(*) FROM usuario_permisos_historial WHERE usuario_id=:id AND cambiado_por=:actor');$q->execute(['id'=>$id,'actor'=>$actor]);$assert((int)$q->fetchColumn()===4,'Cada cambio registra usuario, actor y fecha en historial');
 ...
 $assert(etiquetaRol('administracion_docente')==='Administración_Docente','El nombre visual solicitado se muestra correctamente');
