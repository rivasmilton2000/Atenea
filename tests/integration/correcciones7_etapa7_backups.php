...
try{
    ...
    $download=file_get_contents(dirname(__DIR__,2).'/src/dashboard/backups/descargar.php');$sqlExport=file_get_contents(dirname(__DIR__,2).'/includes/database_sql_export.php');$assert(str_contains($download,"REQUEST_METHOD")&&str_contains($download,'validarTokenCsrf')&&str_contains($download,'crearArchivoSqlDesdeRespaldoAtenea')&&str_contains($sqlExport,'verificarIntegridadRespaldo($respaldo)'),'La descarga exige sesión administrativa, POST, CSRF e integridad antes de convertir a SQL');
